...
then sudo virt-builder centos-6 --output $OS_IMAGE_DIR/c66v311.qcow2 --verbose --format qcow2 --hostname c66v311.com --install openssh-server,syslinux,lsb,sdparm,nc,java-1.7.0-openjdk-devel.x86_64,mysql-connector-java,php-mysql,mysql,mysql-server,rpcbind,createrepo --root-password password:Al3ks3y7! --size 30G --run c66v311.sh
fi

if [[ $OS_TYPE == 'Ubuntu14.04' && $MAPR_VERSION == '4.0.1' ]]
then sudo virt-builder ubuntu-14.04 --output $OS_IMAGE_DIR/u1404v401.qcow2 --verbose --format qcow2 --hostname u1404v401.com --install openssh-server,openjdk-7-jdk,syslinux,lsb,sdparm,dpkg-dev --root-password password:Al3ks3y7! --size 30G --run u1404v401.sh 
fi
